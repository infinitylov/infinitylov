...
import { PROXY_URL, SEND_PROMPT_URL, supabaseHeaders } from './lib/constants.js';
...
const _PULSE_RUNTIME_OK = (() => {
  try {
    ...
    if (!m) return false;
    if (m.name !== 'InfinityLov') return false;
        ...
        case 'SEND_MESSAGE_PROXY': {
          ...
          const buildLastPayload = () => {
            ...
            if (lastCaptureFull && typeof lastCaptureFull === 'object') {
              return { ...lastCaptureFull, ...tpl };
            }
            return tpl;
          };

          const PROXY_HEADERS = supabaseHeaders();
          ...
          for (let attempt = 0; attempt <= 3; attempt++) {
            ...
            try {
              // Espelha o LOV comercial: POST /send-lovable-prompt (não intercepta /chat na página).
              const payload = {
                token,
                projectId,
                message: msgText,
              ...
              proxyResp = await fetch(SEND_PROMPT_URL, {
                method: 'POST',
              ...
              if (proxyResp.status === 401 || proxyResp.status === 403 || proxyResp.status === 402) break;
              // 5xx definitivo (ex: supabase pausado) — NÃO retentar (queima crédito)
              if (proxyResp.status >= 500) break;
          ...
          if (proxyResp.ok || proxyResp.status === 202) {
            const okBody = await proxyResp.clone().json().catch(() => ({}));
            // Defesa: se o servidor devolveu transform por engano, não fingir sucesso.
            if (okBody && okBody.action === 'transform' && okBody.body && okBody.ok !== true) {
              sendResponse({ ok: false, error: 'Servidor devolveu transform em vez de send. Recarregue a extensão.' });
            } else {
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  if (!s.deviceId) {
    ...
    console.log('[InfinityLov] HWID gerado:', deviceId);
  }
...
chrome.runtime.onStartup?.addListener(async () => {
  ...
  if (!s.deviceId) {
    const deviceId = crypto.randomUUID();
    await setSettings({ deviceId });
    console.log('[InfinityLov] HWID gerado no startup:', deviceId);
  }
});
...
chrome.alarms?.onAlarm.addListener(async (alarm) => {
  ...
  try {
    ...
    console.warn('[InfinityLov] Erro transitório no polling de licença:', e?.message || e);
    return;
  }
  if (!state) return;
  ...
  if (state.status === 'transient' || state.status === 'device_mismatch') {
    console.warn('[InfinityLov] polling não-conclusivo (' + state.status + ') — mantendo licença:', state.error);
    return;
  }
  ...
  updateBadge(updated);
  broadcastLicenseRevoked();
  console.log('[InfinityLov] licença revogada/expirada no polling:', state.status, state.error);
});
...
async function doLicenseLogout(errMsg) {
  try {
    ...
    updateBadge(updated);
    broadcastLicenseRevoked();
    console.log('[InfinityLov] logout forçado pelo servidor (licença inválida):', errMsg);
  } catch (e) {
    console.warn('[InfinityLov] falha ao deslogar após rejeição do servidor:', e?.message || e);
  }
}
...
try {
  chrome.webRequest.onSendHeaders.addListener(
    (details) => {
      ...
      if (apiToken) {
        ...
        console.log('[InfinityLov bg] token capturado via webRequest');
      }
    },
  ...
  console.warn('[InfinityLov bg] onSendHeaders indisponível:', e?.message);
}
...
try {
  ...
  console.log('[InfinityLov bg] onBeforeRequest ativo — captura payload nativo do chat');
} catch (e) {
  console.warn('[InfinityLov bg] onBeforeRequest indisponível:', e?.message);
}
