// ============================================================
// InfinityLov Extension - Popup (chat direto)
// Backend local via background.js
// ============================================================
...
// ============ License (via background InfinityLov) ============
...
async function validateLicense(licenseKey) {
  try {
    console.log('[InfinityLov] Validando licença via background:', licenseKey?.substring(0, 8) + '***');
    ...
    console.error('[InfinityLov] Erro ao validar licença:', error);
...
async function sendChatMessage(sessionToken, message, projectId) {
  try {
    ...
    console.error('[InfinityLov] Erro ao enviar mensagem:', error);
    return { status: 'error', message: error.message };
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ============ License Check (via background — formato InfinityLov) ============
  async function checkStoredLicense() {
    // Lê do formato InfinityLov (settings.licenseState) e do legado
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', async () => {
      // Limpa via background (formato InfinityLov)
